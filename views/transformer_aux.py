...
mva = st.sidebar.number_input("Transformer Rating (MVA)", value=p_data["mva"], step=1.0, format="%.3f")

st.sidebar.markdown("**HV Winding (23kV side, Delta) — Multi-Ratio Wye CT**")
...
st.sidebar.markdown("**LV Winding (13.8kV side, Resistance-Grounded Wye) — Delta CT**")
kv_lv = st.sidebar.number_input("LV Rated Voltage (kV)", value=p_data["kv_lv"], step=0.1, format="%.3f")
ct_lv = st.sidebar.number_input("LV CT Ratio (Primary A, e.g. 4000 in '4000:5')", value=p_data["ct_lv"])
ct_conn_lv = st.sidebar.selectbox(
...
with tab_sld:
    ...
    st.caption(
        "Shows where the CTs sit and what falls inside the 87AT differential zone."
    )
    render_zone_diagram(
        "transformer_aux.png",
...
with tab3:
    ...
    comm_chart_units = st.radio("Chart units", ["Per-Unit (pu)", "Secondary Amps (A)"], horizontal=True, key="aux_comm_chart_units")
